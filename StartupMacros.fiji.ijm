...
macro "SaveClicks4D [0]"{
//
// On a z&t stack, will save xy coordinates of the clicks making up
// the polygonal selection delimiting the cell contour at each z & t.
// To do this, just hit the "0" key to save txt files after having
// installed this macro in fiji (Plugins>Macros>Install).
// 
// Version 1.1
// Chloe Roffay
// Boris Guirao
...
// get the path to the directory containing the open image
stackPath = getDirectory("image");

subFile = stackPath+"cell.being.clicked.txt";
filestring=File.openAsString(subFile);
c = parseInt(filestring);

subPath = stackPath+"/cell#"+c;
 
// grab the image title
title = getTitle();
// remove the ".jpg" extension from the string
//T = replace(title, ".tif", "");
...
//entrer a la main si c'est une d ou une v
//vd = "cell4";

// construct the path to save the coordinates
outPath1 = subPath+"/"+"t"+t+"_z"+z+"_xy"+".txt";
outPath2 = subPath+"/"+"t"+t+"_z"+z+"_measure"+".txt";

//outPath = path+"/"+T+"_t"+t+"_z"+z+"_xy"+".txt";
//outPath2 = path+"/"+T+"_t"+t+"_z"+z+"_measure"+".txt";

// save the selection as a text file to image directory
saveAs("XY Coordinates", outPath1);

//Obtain area
run("Measure");
saveAs("Results", outPath2);
selectWindow("Results");
run("Close"); 
 
// print saved record to the log to confirm save
print(File.nameWithoutExtension+".txt");
 
// clear the present selection
//run("Select None");
 
// open the next image in the directory
//run("Open Next");
}

macro "MakeNewFolder [1]"{
//
// Creates new folder with name "cell#XX", the last existing folder corresponding to (XX-1).
// Also creates file "cell.being.clicked.txt" that ONLY contains number "XX".
// 
// Version 1.0
// Boris Guirao
////////////////////////////////////////////////////////////////////////////////////////////////////
	
path = getDirectory("image"); // gets parent directoy

// Initialization
c = 1;
subPath = path+"/cell#"+c;

// Looking for existing folders
 while (File.exists(subPath)){
 c++;							// defining next cell number
 subPath = path+"/cell#"+c;		// defining corresponding path
 }

File.makeDirectory(subPath);	// creating this cell directory

// Saves c value in a txt file
subFile = path+"cell.being.clicked.txt"; // file indicating cell being processed
f = File.open(subFile); 
print(f,c);
}
